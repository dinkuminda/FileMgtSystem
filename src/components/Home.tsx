...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#fafbfc] flex flex-col font-sans selection:bg-[#0b57d0] selection:text-white" id="portal-frame-root">
      ...
      {/* 3. SYSTEM FEATURES INFORMATION LOGS GRID (Overview & Key Capabilities) */}
      <section className="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16 border-t border-slate-100/80" id="portal-archive-overview">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
          
          {/* Left Side: Overview */}
          <div className="lg:col-span-5 text-left space-y-6">
            <span className="text-[10px] font-bold text-blue-600 bg-blue-50 border border-blue-200/60 px-3 py-1.5 rounded-full uppercase tracking-wider font-mono">
              Centralized Repository
            </span>
         

            <div className="p-6 bg-blue-50/40 rounded-2xl border border-blue-100 flex flex-col justify-between">
              <h4 className="text-sm font-extrabold text-[#072146] uppercase tracking-wide mb-1">
                The Value It Delivers
              </h4>
              <p className="text-xs text-[#4a5568] leading-relaxed font-semibold">
                <strong className="text-blue-700 font-bold">Efficiency Meets Security:</strong> By replacing fragmented local folders with a unified digital archive, organizations eliminate data silos, reduce operational downtime spent searching for files, and ensure compliance with modern data retention laws.
              </p>
            </div>
          </div>

          {/* Right Side: Key Capabilities */}
          ...
          <div className="lg:col-span-6 text-left space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div className="p-5 bg-white border border-slate-200/60 rounded-xl space-y-3 shadow-xs">
                <div className="w-9 h-9 bg-blue-50 border border-blue-100 text-blue-600 rounded-lg flex items-center justify-center">
                  <Globe className="w-5 h-5" />
                </div>
                <h4 className="font-bold text-slate-900 text-sm">Systematic Taxonomy &amp; Metadata</h4>
                <p className="text-[12px] text-slate-500 leading-relaxed font-medium">
                  Implements standardized naming conventions and tags, transforming a cluttered "digital junk drawer" into an easily searchable ecosystem.
                </p>
              </div>

              <div className="p-5 bg-white border border-slate-200/60 rounded-xl space-y-3 shadow-xs">
                <div className="w-9 h-9 bg-emerald-50 border border-emerald-100 text-emerald-600 rounded-lg flex items-center justify-center">
                  <Shield className="w-5 h-5" />
                </div>
                <h4 className="font-bold text-slate-900 text-sm">Version Control &amp; Audit Trails</h4>
                <p className="text-[12px] text-slate-500 leading-relaxed font-medium">
                  Tracks file histories and user access, ensuring a single source of truth while protecting against accidental overwrites.
                </p>
              </div>

              <div className="p-5 bg-white border border-slate-200/60 rounded-xl space-y-3 shadow-xs">
                <div className="w-9 h-9 bg-amber-50 border border-amber-100 text-amber-600 rounded-lg flex items-center justify-center">
                  <FileText className="w-5 h-5" />
                </div>
                <h4 className="font-bold text-slate-900 text-sm">Advanced Searchability</h4>
                <p className="text-[12px] text-slate-500 leading-relaxed font-medium">
                  Leverages full-text indexing and metadata filters so users can locate critical documents in seconds.
                </p>
              </div>

              <div className="p-5 bg-white border border-slate-200/60 rounded-xl space-y-3 shadow-xs">
                <div className="w-9 h-9 bg-indigo-50 border border-indigo-100 text-indigo-600 rounded-lg flex items-center justify-center">
                  <Lock className="w-5 h-5" />
                </div>
                <h4 className="font-bold text-slate-900 text-sm">Security &amp; Compliance</h4>
                <p className="text-[12px] text-slate-500 leading-relaxed font-medium">
                  Protects sensitive data through role-based access control (RBAC), encryption at rest and in transit, and automated retention policies.
                </p>
              </div>
