...
import { 
  FileText, Users, LogOut, Plus, Search, Filter, 
  CreditCard, Fingerprint, MapPin, FileQuestion,
  ChevronDown, Download, Trash2, Edit2, Loader2,
  Upload, FileOutput, FileInput
} from 'lucide-react';
...
export default function Dashboard({ userProfile }: DashboardProps) {
  ...
  const [activeTab, setActiveTab] = useState<RecordType>('VISA');
  const [searchQuery, setSearchQuery] = useState('');
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<ImmigrationRecord | null>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const tabs: { type: RecordType; icon: any; label: string }[] = [
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      // Clean up attachments from DB first
      await supabase
    ...
    Papa.parse(file, {
      ...
      complete: async (results) => {
        try {
          ...
          }).filter((row: any) => row.full_name); // Basic validation
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col">
        ...
        <div className="p-4 border-t border-gray-100">
          <div className="bg-gray-50 p-4 rounded-xl mb-4">
            <p className="text-xs text-gray-400 font-semibold uppercase tracking-wider mb-2">User</p>
            <p className="text-sm font-bold text-gray-900 truncate">{userProfile?.full_name || 'Staff Member'}</p>
            <p className="text-xs text-gray-500 mt-1 capitalize">{userProfile?.role} Account</p>
          </div>
          <button 
      ...
      <main className="flex-1 overflow-y-auto">
        <header className="bg-white border-b border-gray-200 px-8 py-4 flex items-center justify-between sticky top-0 z-10 shadow-sm">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 tracking-tight">{activeTab} Management</h2>
